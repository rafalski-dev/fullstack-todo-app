...
export function Panel({ onError, session }: PanelProps) {
  ...
  const completedNumberOfTasks: number = todoData.filter(
    (todo) => todo.done,
  ).length;
  const tobedoneNumberOfTasks: number = todoData.filter(
    (todo) => !todo.done,
  ).length;
  ...
  function switchOffEditing() {
    setTodoData((prevTodo) =>
      prevTodo.map((todo) => ({ ...todo, editing: false })),
    );
  }
  ...
  return (
    <div className={styles.panel}>
      ...
      <Footer
        ...
        tobedoneNumberOfTasks={tobedoneNumberOfTasks}
      />
    </div>
  );
}
